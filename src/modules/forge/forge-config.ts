 ...
 * All env vars are NEXT_PUBLIC_ â€” available both in server components and browser.
 ...
 * MUST be set in .env.local â€” empty string will cause runtime errors on contract calls.
 */
export const FACTORY_CONTRACT_HASH =
  process.env.NEXT_PUBLIC_FACTORY_CONTRACT_HASH ?? "";

/** How long to poll for TX confirmation before giving up (ms). Default: 10 seconds. */
export const TX_POLLING_TIMEOUT_MS = Number(
  process.env.NEXT_PUBLIC_TX_POLLING_TIMEOUT_MS ?? "10000"
);
